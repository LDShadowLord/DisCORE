class Logs:
    ...
    def commit(self, clear=True):
        ...
        """
        ready_to_output = ""
        self._hook.send(content="__Generated by DisCORE 𝖑𝖔𝖌𝖘 "+self.version+"__\n"+self.name)
        for line in self._log_output:
            if len(line)+len(ready_to_output) > 1950:
                self._hook.send(content="```"+ready_to_output+"```")
...
class Notify:
    ...
    def embed(self, description, title, url=None, color=0x7289da, thumbnail=None, image=None, author=None, removeFooter=False):
        ...
        if removeFooter != True:
            exportEmbed.set_footer("Generated by DisCORE 𝓷𝓸𝓽𝓲𝓯𝔂 "+self.version, "https://raw.githubusercontent.com/LDShadowLord/DisCORE/master/DisCORE/assets/logo.png")
